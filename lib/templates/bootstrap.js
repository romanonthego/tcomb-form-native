...
function phoneNumber(locals) {
  ...
  return (
    <View style={formGroupStyle}>
      {label}
      <View styles={stylesheet.phoneNumber.input}>
        <Text styles={stylesheet.phoneNumber.countryCode}>{locals.countryCodes[0] || "+0"}</Text>
        <View styles={stylesheet.phoneNumber.field}>{textbox(locals)}</View>
      </View>
      {help}
      {error}
    </View>
